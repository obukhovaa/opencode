...
func extractInlineCode(s string, counter *int, placeholders map[string]string) string {
	...
	return inlineCodeRe.ReplaceAllStringFunc(s, func(m string) string {
		content := m[1 : len(m)-1]
		token := newPlaceholder(counter)
		placeholders[token] = "<code>" + escapeHTML(content) + "</code>"
		return token
	})
}
	...
	for i < len(lines) {
		...
		if isTableLine(line) {
			...
			if j-i >= 2 {
				...
				out = append(out, "<pre>"+escapeHTML(run)+"</pre>")
				i = j
				continue
			}
		...
		if strings.HasPrefix(trimmed, ">") {
			...
			for j < len(lines) {
				...
				quoteLines = append(quoteLines, processInline(inner))
				j++
			}
			out = append(out, "<blockquote>"+strings.Join(quoteLines, "\n")+"</blockquote>")
			i = j
			continue
		}

		if m := headingRe.FindStringSubmatch(line); m != nil {
			out = append(out, "<b>"+processInline(m[1])+"</b>")
			i++
			continue
		}
		...
		if m := taskItemRe.FindStringSubmatch(line); m != nil {
			...
			out = append(out, m[1]+box+" "+processInline(m[3]))
			i++
			continue
		}

		if m := ulItemRe.FindStringSubmatch(line); m != nil {
			out = append(out, m[1]+"\u2022 "+processInline(m[2]))
			i++
			continue
		}
...
// nesting (e.g. italic inside bold) round-trips correctly.
	...
	for i < n {
		...
		case i+1 < n && (s[i:i+2] == "**" || s[i:i+2] == "__"):
			...
			if inner, newPos, ok := matchDelim(s, i, marker); ok {
				b.WriteString("<b>")
				b.WriteString(processInline(inner))
				b.WriteString("</b>")
				i = newPos
				continue
			}
