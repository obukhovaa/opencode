...
Output: if it exceeds ${maxLines} lines or ${maxBytes} bytes, the full output is saved to a temp file and a truncated preview (first/last 500 lines) is shown — read sections of the saved file with the read tool or search it with the grep tool. Never pre-truncate with ` + "`head`/`tail`" + `; just run the command.
...
- NEVER use destructive flags (push --force, reset --hard), skip hooks (--no-verify, --no-gpg-sign), or amend published commits unless explicitly requested
