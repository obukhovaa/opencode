...
SELECT id, session_id, role, parts, model, seq, created_at, updated_at, finished_at FROM (
    SELECT id, session_id, role, parts, model, seq, created_at, updated_at, finished_at
    FROM messages
    WHERE session_id = ?
...
func (q *Queries) ListLatestMessagesBySession(ctx context.Context, arg ListLatestMessagesBySessionParams) ([]Message, error) {
	rows, err := q.db.QueryContext(ctx, listLatestMessagesBySession, arg.SessionID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
