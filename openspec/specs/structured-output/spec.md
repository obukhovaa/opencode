...
## Purpose
Defines the schema-conformance contract of the injected `struct_output` tool: given a user-supplied output schema, the tool reconciles the model's emitted JSON against that schema before returning it — materializing declared defaults for omitted fields and rejecting a call that is missing a required field with no default. This keeps the returned output complete so downstream consumers (notably flow routing, which derives its args from the returned JSON) never see a defaulted field as a missing key, and forces a retry rather than persisting a genuinely incomplete result.
