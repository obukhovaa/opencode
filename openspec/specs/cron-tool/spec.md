...
Defines the cron scheduler's integration with the shared task-notifications primitive. Cron's `writeSyntheticMessages` delegates to `task.EnqueueTaskCompletion`, producing observationally identical Assistant(ToolCall) + Tool(ToolResult) pairs as before — with one addition: the Assistant message carries `synthetic: true`. Cron retains ownership of its own session-busy lock semantics; the shared primitive does not acquire it on cron's behalf. The cron permission-gating behavior at job-creation time is unchanged; cron-fired completions do not trigger new permission prompts.
