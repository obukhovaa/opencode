# struct-output-schema-delivery Specification

## Purpose
Governs where a flow step's structured-output JSON Schema is placed in the
request the provider builds, and what the `struct_output` tool accepts and
enforces as a result.

Anthropic renders the cacheable prefix as `tools` -> `system` -> `messages` and
invalidates everything after the first changed byte. A `struct_output` tool
whose parameters are derived from the step's schema therefore puts a per-step
payload at position 0 of that prefix, so two consecutive steps of one agent
share no cache at all — not the tool list, not the system prompt, and not even
the message history a forked session just copied verbatim. The intent here is a
tool definition that is byte-identical across steps, with the schema delivered
after the last cache breakpoint, and without weakening what the tool validates
or what its result looks like to downstream consumers.

The schema-conformance contract itself — default materialization and
required-field rejection — lives in `openspec/specs/structured-output/spec.md`
and is delivery-independent; this capability only governs where the model is
shown the schema.
