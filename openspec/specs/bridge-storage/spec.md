...
Defines the persistence layer for the chat bridge: dual-provider goose migrations (SQLite + MySQL) for `bridge_sessions` and `bridge_allowlist`, project-scoped composite primary keys, the `mention_handle` / `mention_consumed_at` columns, the `ON DELETE SET NULL` FK from `bridge_sessions.session_id` to `sessions(id)` with its mandatory supporting index, sqlc query files per provider, and the single-writer election mechanism (SQLite file lock + MySQL `GET_LOCK`). Establishes the type-mapping invariants that keep the InnoDB key-length budget under 3072 bytes.
