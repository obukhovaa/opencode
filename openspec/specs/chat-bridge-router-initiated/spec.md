# Chat Bridge Router-Initiated Conversations

## Purpose

Defines the binding-first conversation model: `bridge.Service.Bind` / `Unbind` as in-process APIs, the flow engine's auto-bind on entering an interactive step, peer-id resolution (Slack user → DM channel, Mattermost user → DM channel), first-outbound binding mutation (Slack `C…|ts`, Mattermost `<channelId>|<rootPostId>`), and the mention-prefix-on-first-message semantics tracked via `mention_consumed_at`. Router-initiated is the primary path for c2-agent interactive flows — peers do not have to message the bot first.
