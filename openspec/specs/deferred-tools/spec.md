...
## Purpose
On-demand tool loading: agents opt in (via `deferredTools`) to marking enabled
tools as *deferred* so their schemas stay out of the request payload until the
model discovers them via tool search. Covers the config surface, the
`toolsearch` tool, the native (Anthropic server-side tool search, cache-
preserving, single-turn) and fallback (client-side, two-turn) activation
paths, per-session activation and announcement state, token accounting, and
cache-stability guarantees. Tuned to keep large MCP tool fleets out of context
until needed, with no behavior change for agents that don't opt in.
