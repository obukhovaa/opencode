# Config Atomic Writeback

## Purpose

Defines the exported `config.UpdateCfgFile` helper used by every persistor of `.opencode.json` (TUI, bridge, agent-model selector). Replaces the prior in-place `os.WriteFile` with the canonical POSIX safe-replace sequence (temp file → fsync → rename → parent-dir fsync) so SIGKILL or power-loss leaves either the prior or new contents, never a truncated file. Encodes the file-mode policy that auto-tightens to `0o600` on tokens but preserves existing hardened modes. Documents that in-memory `cfg` mutation remains the caller's responsibility.
