...
Scoped context path resolution at agent and flow-step granularity. Covers the
`context: { paths, mode, nested }` config surface on agents and steps, shell-free
templating with workDir containment enforcement, keyed memoization, `replace`/`append`
precedence semantics, and backward-compatibility guarantees.
