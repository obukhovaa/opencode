...
Rationale: while the owning step's Run is in flight, the step's end-of-turn drain (`WaitForActiveTasks`) is the sanctioned reaction mechanism. After the step has ended, the flow has already routed on the step's result — an auto-resume would start a zombie turn on a completed step's session, and (sessions record no owning agent) it would run under the active/primary agent rather than the step's agent (GENAI-239).
