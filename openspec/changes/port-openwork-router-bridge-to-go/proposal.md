...
This change also folds in two adjacent gaps that any non-trivial use of the bridge by the c2 agent orchestrator (`/Users/nouwa/Development/piano/c2-agent`) hits today:
...
- **External consumers to update in sync**:
   ...
   - The c2 agent orchestrator at `/Users/nouwa/Development/piano/c2-agent` — its k8s Job spec needs the dual-container model (opencode + no second router process), reads from `/flow/*` SSE for progress, calls `/router/bind` (or relies on flow-args auto-bind).
