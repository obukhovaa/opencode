...
- **Malformed / errored struct_output is out of scope.** The gate fires only when there is NO struct_output (nil or empty content) — matching the pre-existing loop validation. If the agentic turn emits a *schema-invalid* struct_output, `captureStructOutput` records it as an error `ToolResult` with non-empty content ("Invalid JSON: …"), which slips past the gate and is accepted as-is (downstream `json.Unmarshal` then silently merges no args). This is the same behavior as before this change, but it is the identical silent-strand failure mode, so extending the gate to also re-force on `result.StructOutput.IsError` is a natural follow-up (the forcing helper already rejects error results internally, so only the outer gate needs widening).
