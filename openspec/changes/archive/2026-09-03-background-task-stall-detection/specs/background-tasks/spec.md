...
Delta spec for the `background-task-stall-detection` change. Restates only the
requirement that changes; unchanged requirements are not repeated here. For the
...
therefore ends either when the surrounding `ctx` is done or when the task is
terminated as stalled, whichever comes first — so the deadline sources for the
hold are `Step.Timeout`, `OPENCODE_NON_INTERACTIVE_TASK_WAIT_TIMEOUT`, and
`backgroundTasks.stallThreshold`.

The sentence above stating that the wait imposes no timeout of its own remains
in force for the wait itself: it still blocks until every pending task reaches a
terminal state, and it never returns early on a task that is still pending. What
changed is when a subagent task can reach that terminal state. In particular no
task that is making progress is ever terminated, regardless of total runtime,
and `bash`, `monitor` and `cron` tasks are never terminated on progress grounds
at all.
