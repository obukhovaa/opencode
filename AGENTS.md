...
- `context`: Scoped context resolution — `{"paths": ["AGENTS.runtime.md"], "mode": "replace", "nested": false}`. `paths` overrides the global `contextPaths` for this agent: `mode: replace` (the default when `paths` is declared) excludes the lower layers, `mode: append` concatenates after them; precedence is step > agent > global, and each declaring layer's own `mode` decides whether the layers below contribute. Path entries support `${agent}`, `${flow.id}`, `${flow.step}`, and `${env.VAR}` tokens (shell-free; entries with an unknown token or an empty expansion are skipped) and must resolve inside the working directory. `nested: false` opts the agent out of the nested-context manifest and tool-result injection. Flow steps accept the same object per step (`context` in the step YAML, inheritable via `extends`). Nested-file discovery is governed by the top-level `contextDiscovery` object (`enabled`, `maxFiles`, `maxDepth`, `maxFileBytes`, `maxSessionBytes`). With no `context` declared anywhere the assembled prompt is byte-identical to the pre-feature behavior. Full docs: [docs/context.md](docs/context.md).
