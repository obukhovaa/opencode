...
- **Template keys are checked by a two-part rule, not an allow-list of inheritable fields.** A key must (1) be a known `Step` field — so `promt:` is a load error listing the real fields, instead of being silently dropped as a typed decode drops it — **and** (2) not be one of `id`, `interactive`, `interaction`, `resume_after`, each of which errors quoting *why*. Those reasons are load-bearing and must be kept true: `resume_after` was justified as "not modelled by this engine" until v0.13.19 added it, so the rejected set is justified by WHO READS a key, not by which struct models it. `agent` and `prompt` are inheritable but appear in some consumers' step listings, which will show an empty prompt for an inherited one — known and accepted. The rule covers top-level keys only. Consequence, and the point of the shape: **a field added to `Step` later is inheritable by default.** An allow-list would put the maintenance friction on the fast-growing side — this engine's `Step` gains fields regularly while the orchestrator's small struct almost never changes — so every new engine field would be silently non-inheritable until someone curated a list in `include.go`.
...
- **The key derivation (`yamlFieldIndexes`) mirrors yaml.v3 in two ways that both matter.** It skips **unexported** fields: mapping one would make `reflect.Value.Set` panic (`reflect: reflect.Value.Set using value obtained using unexported field`), and since nothing between `parseFlowFile` and `discoverFlows` recovers, that is a process death during flow discovery rather than a skipped flow — reached by the ordinary act of adding an unexported tagged field to `Step`. It also falls back to the **lowercased field name** for an exported field with no yaml tag, because yaml.v3 does: without the fallback such a field decodes fine inline (`retries: 3`) yet is rejected in a template, quietly voiding "inheritable by default". `Step` has neither shape today, so the derivation is split out of the `OnceValue` and tested against a probe type — otherwise both rules could be deleted with the suite green. `mergeTemplates` also checks `CanSet()` as belt and braces.
- **Inherited values are deep-copied** (`deepCopyValue`, generic by reflection for the same reason the merge is). Without it two steps extending one template share the `*StepOutput`, its `Schema` map, the `Rules` backing array, `*Fallback` and `*Compact`. Nothing mutates those after load today, but flows sit in the process-wide `flowCache` shared by every concurrent job, so the first per-step mutation anyone adds becomes cross-step **and** cross-job corruption — and `-race` stays quiet while nothing writes.
- **A template's `output.schema` may not be a chained `$ref`.** `format.ResolveSchemaRef` resolves exactly one level, so a schema file that is itself a bare `{"$ref": …}` would have its second hop resolved by the flow's own loop against the **flow's** directory, breaking the template-relative rule and picking the wrong file when a same-named schema sits beside the flow. `decodeStepTemplate` rejects a still-present `$ref` after resolution.
- **A template must declare at least one key.** A null template (`.empty:` with no body) would otherwise merge an empty agent and prompt into the step, and `validateFlow` does not require a prompt — so the step would load and run empty.
