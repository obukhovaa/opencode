...
Always define a fallback step for flows where reliability matters. A common pattern is a terminal `failed` step that summarizes the error. `fallback.to` edges must not form a cycle (`a → b → a`, or a step falling back to itself) — the flow is rejected at load. A fallback target may still be re-entered through a `cycle: true` rule (fail → escalate → cycle back → fail → escalate again); the runtime admits that but caps fallback-origin entries per step per run at `flow.maxFallbackEntries` (default 3) and fails the run when the cap trips, so keep escalation ladders short. See `references/flow-spec.md` → "Fallback".
